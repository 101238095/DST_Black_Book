...
--  type( TUNING["The_Black_Book.Language"] ) is "function" or "string"
-- 
--  If anyone wants to add additional language support themselves, 
--  please add this function before this mod is loaded (set a higher priority)  : TUNING["The_Black_Book.Language"]
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
TUNING["The_Black_Book.Language"] = TUNING["The_Black_Book.Language"] or function()
	-- return "en"
	local language = "en"
	pcall(function()
		language = TUNING["The_Black_Book.Config"].Language
...
-- 		    local LANGUAGE = type(TUNING["The_Black_Book.Language"]) == "function" and TUNING["The_Black_Book.Language"]() or TUNING["The_Black_Book.Language"]
-- 		    return TUNING["The_Black_Book.Strings"][LANGUAGE][prefab_name] or {}
-- 		end
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-- 		-- if modname == "The_Black_Book" then -- 如果MOD文件夹名字是 "The_Black_Book"
...
modimport("Imports_for_BLACK_BOOK/__All_imports_init.lua")	---- 所有 import  文本库（语言库），素材库

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- MOD屏蔽
	-- modimport("modblocker.lua")
	-- if TUNING["The_Black_Book.Mod_Blocker"] then
	-- 	return
	-- end
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- 获取mod 版本
	...
	TUNING["The_Black_Book.mod_info"] = mod_info
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


modimport("Key_Modules_Of_BLACK_BOOK/_All_Key_Modules_Init.lua")	---- 载入关键功能模块,在 prefab 加载之前，方便皮肤的API HOOK

PrefabFiles = {  "the_black_book__all_prefabs"  }		---- 通过总入口 加载所有prefab。
